/* ==========================================================
   6ol · Ritual Core · v2.0
   Gate v2: Loop Ascension + Passphrase Unlock
   ========================================================== */
...
function setLoop(level) {
  userLoopLevel = level;
  localStorage.setItem('loopLevel', String(level));
  updateLoopDisplay();
  renderAll();
}
function updateLoopDisplay() {
  const loopName = ['Initiate', 'Seeker', 'Witness', 'Architect', 'Lightbearer'][userLoopLevel - 1] || `Loop ${userLoopLevel}`;
  document.getElementById('loopStatus').textContent = `Current Loop: ${userLoopLevel} – ${loopName}`;
}
...
statusDiv.innerHTML = `
  ...
  <form id="unlockForm" style="margin-top:1rem">
...
`;
listEl.parentElement.prepend(statusDiv);
updateLoopDisplay();
